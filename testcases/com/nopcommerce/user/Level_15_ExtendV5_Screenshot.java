...
public class Level_15_ExtendV5_Screenshot extends BaseTest {
	...
	public void User_01_Register(Method method) {
		...
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 02: Enter to First Name text box with value is '"+ firstName+ "'");
		registerPage.inputToFirstNameTextbox(firstName);
		
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 03: Enter to last name text box with value is '"+ lastName + "'");
		registerPage.inputToLastNameTextbox(lastName);
		
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 04: Enter to Email address text box with value is '"+ emailAddress + "'");
		registerPage.inputToEmailTextbox(emailAddress);
		
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 05: Enter to password text box with value is '"+ password+ "'");
		registerPage.inputToPasswordTextbox(password);
		
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 06: Enter to confirm password text box with value is '"+ password+ "'");
	...
	public void User_02_Login(Method method) {
		...
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 00: Click to logout link");
		homePage = registerPage.clickToLogoutlink();
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 01: Navigate to 'Login page'");
		loginPage = homePage.openUserLoginPage();
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 02: Enter to email address with value is '" + emailAddress + "'");
		loginPage.InputToEmailTextbox(emailAddress);
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 03: Enter to password with value is '" + password + "'");
		loginPage.InputToPasswordTextbox(password);
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 04: Click to Login Button");
		loginPage.clickToLoginButton();
		ExtentTestManager.getTest().log(Status.INFO,"Register - Step 05: Verify Account display");
